...
// 公开：提交入驻申请
router.post('/', (req, res) => {
  const { company_name, industry, contact_name, contact_phone, contact_email, product_title, product_name, description, product_desc } = req.body;
  ...
  const item = {
    ...
    phone: contact_phone,
    email: contact_email || '',
    product_name: product_name || product_title || '入驻咨询',
    product_desc: product_desc || description || '',
    status: 'pending',
...
router.get('/', authCheck, (req, res) => {
  ...
  // 兼容字段映射：数据库列名 → 前端期望字段名
  res.json(rows.map(r => ({
    ...r,
    contact_name: r.contact_name || r.contact_person || '',
    contact_phone: r.contact_phone || r.phone || '',
    contact_email: r.contact_email || r.email || '',
    product_title: r.product_title || r.product_name || '',
    product_type: r.product_type || '',
    description: r.description || r.product_desc || '',
    company_desc: r.company_desc || '',
  })));
});
...
router.put('/:id/review', authCheck, (req, res) => {
  ...
  // 通知申请人审核结果
  const _title = item.product_name || item.product_title || '';
  const _phone = item.phone || item.contact_phone || '';
  createNotification({
    type: 'onboarding',
    title: status === 'approved' ? '入驻审核通过' : '入驻审核未通过',
    content: status === 'approved'
      ? `恭喜！您的入驻申请「${_title}」已审核通过，即将上线展示。`
      : `您的入驻申请「${_title}」未通过审核。${reject_reason ? '原因：' + reject_reason : ''}`,
    target_phones: [_phone],
  ...
  if (_phone) {
    try {
      ...
      // 通过手机号查找用户openid
      const user = db2.prepare('SELECT openid FROM users WHERE phone = ?').get(_phone);
      if (user && user.openid) {
        subscribeMsg.notifyUser(
          user.openid,
          'audit_result',
          {
            thing1: { value: _title.substring(0, 20) },
...
router.post('/:id/convert', authCheck, (req, res) => {
  ...
  if (!client) {
    ...
    client = {
      ...
      avatar: '🏢',
      cover: '',
      description: (onboarding.product_desc || onboarding.company_desc) || `${onboarding.company_name} 入驻企业`,
      qualifications: [],
      team_size: '',
      founded: '',
      wechat_account_id: '',
      contact_phone: onboarding.phone || onboarding.contact_phone || '',
      contact_email: onboarding.email || onboarding.contact_email || '',
      address: '',
      website: '',
      published: 1,
      created_at: now,
  ...
  const clientProduct = {
    ...
    title: onboarding.product_name || onboarding.product_title || '',
    subtitle: (onboarding.product_desc || '') ? onboarding.product_desc.slice(0, 30) : '',
    category: onboarding.industry || '其他',
    price: '',
    price_unit: '面议',
    image: 'https://images.unsplash.com/photo-1497366216548-37526070297c?w=600',
    tags: onboarding.industry ? JSON.stringify([onboarding.industry]) : '[]',
    description: onboarding.product_desc || '',
    contact_phone: onboarding.phone || onboarding.contact_phone || '',
    published: 1,
    created_at: now,
    updated_at: now
